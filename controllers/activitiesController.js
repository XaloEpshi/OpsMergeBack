...
exports.getAllActivities = async (req, res) => {
    try {
        ...
        const [inventarioBodegas] = await pool.query('SELECT "Inventario Bodegas" AS origen, fecha_inventario AS fecha_hora, nombre_bodega AS actividad, detalle_inventario AS descripcion, responsable AS user_id FROM InventarioBodegas');
        const [exportaciones] = await pool.query('SELECT "Exportaciones" AS origen, fechaCarga AS fecha_hora, CONCAT(material, " exportado a ", mercado) AS actividad, observacion AS descripcion, responsable AS user_id FROM exportaciones');
